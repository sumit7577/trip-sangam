...
import {
  ArrowLeft, CalendarDays, Users, Phone, Clock,
  IndianRupee, CalendarClock, UserCheck, X,
} from "lucide-react";
import type { LucideIcon } from "lucide-react";
import { useAuth } from "@/lib/auth";
import { useModal } from "@/lib/modal";
import { toast } from "@/lib/toast";
import { PriceSummary, OffersStrip, PaymentMethods } from "@/components/trips/CheckoutPanel";
import {
    ...
    <Shell wide>
      ...
      {dep ? (
        <div className="mt-6 grid gap-6 lg:grid-cols-3 lg:items-start">
          {/* LEFT — trip details, who's going, offers, payment options */}
          <div className="space-y-6 lg:col-span-2">
            {/* Matched slot — futuristic seat-meter panel */}
            <div className="relative overflow-hidden rounded-3xl border border-line bg-gradient-to-br from-white to-sand/50 p-4 shadow-soft dark:border-white/10 dark:from-white/[0.07] dark:to-transparent sm:p-6">
...
function Shell({ children, wide }: { children: React.ReactNode; wide?: boolean }) {
  return (
    <main className="min-h-screen bg-sand px-4 pb-24 pt-28 dark:bg-[#0E0E0D] md:pt-32">
      <div className={`mx-auto ${wide ? "max-w-5xl" : "max-w-2xl"}`}>{children}</div>
    </main>
  );
}
